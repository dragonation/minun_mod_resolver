...
LightingLUT.REFLECT_B = 4;
LightingLUT.REFLECT_G = 5;
LightingLUT.REFLECT_R = 6;
...
Submesh.prototype.loadData = function (reader) {
     ...
     this.loadVertexData(reader);
     this.loadIndexData(reader);

};

Submesh.prototype.loadVertexData = function (reader) {

    const reads = {};
    reads[PICA.AttributeFormat.INT_8] = "readInt8";
    reads[PICA.AttributeFormat.UINT_8] = "readUint8";
    reads[PICA.AttributeFormat.INT_16] = "readInt16";
    reads[PICA.AttributeFormat.FLOAT_32] = "readFloat32";

    const procedures = {};
    procedures[PICA.AttributeName.POSITION] = (vertex, vector, attribute) => {
        vertex.position = [vector.x, vector.y, vector.z];
    };
    procedures[PICA.AttributeName.NORMAL] = (vertex, vector, attribute) => {
        vertex.normal = [vector.x, vector.y, vector.z];
    };
    procedures[PICA.AttributeName.TANGENT] = (vertex, vector, attribute) => {
        vertex.tangent = [vector.x, vector.y, vector.z];
    };
    procedures[PICA.AttributeName.COLOR] = (vertex, vector, attribute) => {
        let r = vector.x, g = vector.y, b = vector.z, a = vector.w;
        vertex.color = [a, r, g, b];
    };
    procedures[PICA.AttributeName.TEXTURE_COORDINATE_0] = (vertex, vector, attribute) => {
        if (!vertex.textures) { vertex.textures = []; }
        vertex.textures[0] = [vector.x, vector.y];
    };
    procedures[PICA.AttributeName.TEXTURE_COORDINATE_1] = (vertex, vector, attribute) => {
        if (!vertex.textures) { vertex.textures = []; }
        vertex.textures[1] = [vector.x, vector.y];
    };
    procedures[PICA.AttributeName.TEXTURE_COORDINATE_2] = (vertex, vector, attribute) => {
        if (!vertex.textures) { vertex.textures = []; }
        vertex.textures[2] = [vector.x, vector.y];
    };
    procedures[PICA.AttributeName.BONE_INDEX] = (vertex, vector, attribute) => {
    ...
    procedures[PICA.AttributeName.BONE_WEIGHT] = (vertex, vector, attribute) => {
    ...
    while (looper < this.vertices.count) {
        ...
        while (looper2 < this.vertices.attributes.length) {
            ...
            if (!attribute.fixed) {
                ...
                let read = reads[attribute.format.type.code];
                ...
                procedures[attribute.name.code](vertex, vector, attribute);
...
Submesh.prototype.loadIndexData = function (reader) {
