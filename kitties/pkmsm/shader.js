    ...
    if ((report.type === "geometry") || (report.type === "vertex")) {
        if (glsl) {
            ...
            return [
                ...
                "",
                Object.keys(registers).filter((key) => key[0] === "o").sort(@.cmp.natural).map((register) => {
...
Shader.prototype.decodeFragmentColors = function (material, features, colors) {
    ...
    if (bumpEnabled) {
        // make it -1 to 1
        let bumpColor = `textures[${material.bumpTexture}].rgb * 2.0 - 1.0`;
        if (bumpTangent) {
            codes.push("vec3 surfaceNormal = vec3(0.0, 0.0, 1.0);");
            codes.push(`vec3 surfaceTangent = normalize(${bumpColor});`);
        } else {
            codes.push(`vec3 surfaceNormal = ${bumpColor};`);
            // sometimes the normal extract from normal map is all zero
            //     which is strange. maybe some mistakes by Pokemon teams
            //     so I restore them from input[normal].
            // there's still possibility which the normal is abandon by
            //     them, so I don't know whether I should do this.
            codes.push("if ((length(surfaceNormal) < 0.1) || useDirectNormal) {");
            codes.push("    surfaceNormal = fragNormal.rgb;");
            codes.push("}");
            codes.push("surfaceNormal = normalize(surfaceNormal);");
    ...
    if (features.fresnel) {
        if (features.fresnelPrimary) {
            codes.push(`        lutColor.a = ${fresnel};`);
        } else if (features.fresnelSecondary) {
...
Shader.prototype.getLightingLUTInput = function (selection, scale, lut) {
    ...
    let selectionCode = selection.code;
    ...
    switch (scale.code) {
        ...
        case PICA.LightingLUTInputScaleMode.EIGHT: {  output = `min(${output} * 8.0, 1.0)`; break; }
